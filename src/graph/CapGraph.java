...
public class CapGraph implements Graph {
	...
	public Graph getEgonet(int center) {
		...
		for (Integer neighbor : neighbors) {
			...
			for (Integer neighborOfNeighor : graph.get(neighbor)) {
	...
	public Stack<Integer> DFS(Stack<Integer> vertices, Graph graphToSearch, List<Stack<Integer>> listOfListOfNodesInSCCs) {
		...
		while (!vertices.isEmpty()) {
			...
			if (!visited.contains(vertex)) {
				...
				DFSVisit(vertex, visited, finished, graphToSearch, exploredVerticesInThisRun);
	...
	public void DFSVisit(Integer vertex, HashSet<Integer> visited, Stack<Integer> finished, Graph graphToSearch,
			Stack<Integer> exploredVerticesInThisRun) {
		visited.add(vertex);
		HashSet<Integer> neighbors = graphToSearch.exportGraph().get(vertex);
		...
		for (Integer neighbor : neighbors) {
			if (!visited.contains(neighbor)) {
				...
				DFSVisit(neighbor, visited, finished, graphToSearch, exploredVerticesInThisRun);
			}
		}
		finished.add(vertex);
	...
	public Graph getTransposedGraph() {
		...
		for (Map.Entry<Integer, ArrayList<Integer>> entry : graph.entrySet()) {
			...
			transposedGraph.addVertex(node);
	...
	public HashMap<Integer, HashSet<Integer>> exportGraph() {
		...
		for (Map.Entry<Integer, ArrayList<Integer>> entry : graph.entrySet()) {
			graphHashMap.put(entry.getKey(), new HashSet<Integer>(entry.getValue()));
		}
		return graphHashMap;
	}
